...
class PriorityListProcessorImpl constructor(
    ...
    private val networkChangeListener: NetworkInfoProvider.NetworkChangeListener =
        object : NetworkInfoProvider.NetworkChangeListener {
            ...
            if (context != null && intent != null) {
                when (intent.action) {
                    ACTION_QUEUE_BACKOFF_RESET -> {
                        if (!stopped && !paused && namespace == intent.getStringExtra(
                                EXTRA_NAMESPACE
                            )
                        ) {
                            resetBackOffTime()
                    ...
                    for (index in 0..priorityList.lastIndex) {
                        if (downloadManager.canAccommodateNewDownload() && canContinueToProcess()) {
                            ...
                            if ((isFetchServerRequest || networkInfoProvider.isNetworkAvailable) && canContinueToProcess()) {
                                ...
                                val properNetworkConditions =
                                    networkInfoProvider.isOnAllowedNetwork(networkType)

...
class ParallelFileDownloaderImpl(
    private val initialDownload: Download,
    ...
    override fun run() {
        ...
        try {
            ...
            downloadInfo.total = total
            val openingRequest =
                if (downloader.getHeadRequestMethodSupported(getRequestForDownload(initialDownload))) {
                    ...
                    if (!interrupted && !terminated) {
                        ...
                        if (!interrupted && !terminated) {
                            ...
                            downloadBlocks.forEach { downloadBlock ->
                                delegate?.onDownloadBlockUpdated(
                                    downloadInfo,
                                    downloadBlock,
                                    totalDownloadBlocks
                                )
                        ...
                        if (sliceFileDownloadsList.isNotEmpty()) {
                            executorService =
                                Executors.newFixedThreadPool(sliceFileDownloadsList.size)
                        }
            ...
            if (!isDownloadComplete() && !terminated && !interrupted) {
                ...
                if (hashCheckingEnabled) {
                    if (openingResponse != null && downloader.verifyContentHash(
                            openingResponse.request,
                            openingResponse.hash
                        )
                    ) {
                        deleteAllInFolderForId(downloadInfo.id, fileTempDir)
                        if (!interrupted && !terminated) {
                            downloadInfo.etaInMilliSeconds = estimatedTimeRemainingInMilliseconds
                            downloadInfo.downloadedBytesPerSecond =
    ...
    private fun waitAndPerformProgressReporting() {
        ...
        while (actionsCounter != actionsTotal && !interrupted && !terminated) {
            ...
            val downloadSpeedCheckTimeElapsed = hasIntervalTimeElapsed(
                downloadSpeedStartTime,
            ...
            if (downloadSpeedCheckTimeElapsed) {
                ...
                estimatedTimeRemainingInMilliseconds =
                    calculateEstimatedTimeRemainingInMilliseconds(
            ...
            val hasReportingTimeElapsed = hasIntervalTimeElapsed(
                reportingStartTime,
    ...
    private fun downloadSliceFiles(
        request: Downloader.ServerRequest,
        fileSlicesDownloadsList: List<FileSlice>
    ) {
        actionsCounter = 0
        ...
        if (!storageResolver.fileExists(request.file)) {
            storageResolver.createFile(
                request.file,
                initialDownload.enqueueAction == EnqueueAction.INCREMENT_FILE_NAME
            )
        }
                ...
                executorService?.execute {
                    try {
                        Thread.currentThread().name =
                            "${downloadInfo.namespace}-${downloadInfo.id}-Slice-${fileSlice.position}"
                    } catch (e: Exception) {
                    ...
                    val downloadRequest = getRequestForDownload(
                        download = downloadInfo,
                        rangeStart = fileSlice.startBytes + fileSlice.downloaded,
                        segment = fileSlice.position + 1
                    )
                    var downloadResponse: Downloader.Response? = null
                    var saveRandomAccessFile: RandomAccessFile? = null
                    try {
                        val file = getFile(
                            getDownloadedInfoFilePath(
                                fileSlice.id,
                                fileSlice.position,
                                fileTempDir
                            )
                        ...
                        if (!terminated && !interrupted && downloadResponse?.isSuccessful == true) {
                            ...
                            var remainderBytes: Long =
                                naturalEnd - (fileSlice.startBytes + fileSlice.downloaded)
                            var reportingStartTime = System.nanoTime()
                            var streamBytes: Int
                            ...
                            while ((totalUnknown || remainderBytes > 0L) && read != -1 && !interrupted && !terminated) {
                                ...
                                if (!interrupted && !terminated && read != -1) {
                                    read = downloadResponse.byteStream?.read(buffer, 0, bufferSize)
                                        ?: -1
                                    if (!totalUnknown) {
                                        remainderBytes =
                                            naturalEnd - (fileSlice.startBytes + fileSlice.downloaded)

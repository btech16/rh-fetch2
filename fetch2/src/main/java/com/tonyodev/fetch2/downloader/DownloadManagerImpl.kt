...
class DownloadManagerImpl(
    ...
    override fun start(download: Download): Boolean {
        return synchronized(lock) {
            ...
            if (downloadCounter >= concurrentLimit) {
                logger.d(
                    "DownloadManager cannot init download $download because " +
                    ...
                    try {
                        ...
                        val runDownload = synchronized(lock) {
                            if (currentDownloadsMap.containsKey(download.id)) {
                                ...
                                downloadManagerCoordinator.addFileDownloader(
                                    download.id,
                                    fileDownloader
                                )
    ...
    private fun getFileDownloader(
        ...
        return if (downloader.getRequestFileDownloaderType(
                request,

...
open class FetchImpl constructor(
    ...
    override fun insertDownloadInfo(
        id: Int,
        url: String,
        fileName: String,
        tag: String,
        ...
        return synchronized(lock) {
            ...
            handlerWrapper.post {
                try {
                    val download = fetchHandler.insertDownloadInfo(id, url, fileName, tag)
                    if (func != null) {
                        uiHandler.post {
                            func.call(download)
                        }
                    ...
                    logger.e("Failed to insert download", e)
                    val error = getErrorFromMessage(e.message)
                    error.throwable = e
                    if (func2 != null) {
                        uiHandler.post {
    ...
    override fun updateCompletedDownloadFile(
        id: Int,
        newFileName: String,
        tag: String,
        func: Func<Download>?,
        ...
        return synchronized(lock) {
            ...
            handlerWrapper.post {
                try {
                    val download = fetchHandler.updateCompletedDownloadFile(id, newFileName, tag)
                    if (func != null) {
                        uiHandler.post {
                            func.call(download)
                        }

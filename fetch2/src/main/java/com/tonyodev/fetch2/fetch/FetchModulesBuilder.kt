...
object FetchModulesBuilder {
    ...
    fun buildModulesFromPrefs(fetchConfiguration: FetchConfiguration): Modules {
        return synchronized(lock) {
            ...
            val modules = if (holder != null) {
                Modules(
                    fetchConfiguration,
                    holder.handlerWrapper,
                    holder.fetchDatabaseManagerWrapper,
                    holder.downloadProvider,
                    holder.groupInfoProvider,
                    holder.uiHandler,
                    holder.downloadManagerCoordinator,
                    holder.listenerCoordinator
                )
            } else {
                val newHandlerWrapper = HandlerWrapper(
                    fetchConfiguration.namespace,
                    fetchConfiguration.backgroundHandler
                )
                ...
                val downloadManagerCoordinator =
                    DownloadManagerCoordinator(fetchConfiguration.namespace)
                val groupInfoProvider =
                    GroupInfoProvider(fetchConfiguration.namespace, downloadProvider)
                ...
                holderMap[fetchConfiguration.namespace] = Holder(
                    newHandlerWrapper,
                    databaseManagerWrapper,
                    downloadProvider,
                    groupInfoProvider,
                    ...
                    downloadManagerCoordinator,
                    listenerCoordinator,
                    newModules.networkInfoProvider
                )
                newModules
            }
    ...
    class Modules constructor(
        ...
        val networkInfoProvider =
            NetworkInfoProvider(fetchConfiguration.appContext, fetchConfiguration.internetCheckUrl)
        ...
        init {
            ...
            fetchDatabaseManagerWrapper.delegate =
                object : FetchDatabaseManager.Delegate<DownloadInfo> {
                    override fun deleteTempFilesForDownload(downloadInfo: DownloadInfo) {
                        val tempDir = fetchConfiguration.storageResolver
                            .getDirectoryForFileDownloaderTypeParallel(

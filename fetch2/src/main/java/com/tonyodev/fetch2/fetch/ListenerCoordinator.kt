...
class ListenerCoordinator(
    ...
    private val fetchGroupListenerMap =
        mutableMapOf<Int, MutableSet<WeakReference<FetchGroupListener>>>()
    ...
    val mainListener: FetchListener = object : FetchListener {
        ...
        override fun onAdded(download: Download) {
            synchronized(lock) {
                ...
                if (fetchGroupListenerMap.isNotEmpty()) {
                    ...
                    val fetchGroup =
                        groupInfoProvider.getGroupReplace(groupId, download, Reason.DOWNLOAD_ADDED)
                    ...
                    groupInfoProvider.postGroupReplace(
                        download.group,
                        download,
        ...
        override fun onQueued(download: Download, waitingOnNetwork: Boolean) {
            synchronized(lock) {
                ...
                if (fetchGroupListenerMap.isNotEmpty()) {
                    val groupId = download.group
                    val fetchGroup =
                        groupInfoProvider.getGroupReplace(groupId, download, Reason.DOWNLOAD_QUEUED)
                    fetchGroupListenerMap.values.forEach {
                        val iterator = it.iterator()
                        while (iterator.hasNext()) {
                            val fetchListener = iterator.next().get()
        ...
        override fun onWaitingNetwork(download: Download) {
            synchronized(lock) {
                ...
                if (fetchGroupListenerMap.isNotEmpty()) {
                    val groupId = download.group
                    val fetchGroup = groupInfoProvider.getGroupReplace(
                        groupId,
                        download,
        ...
        override fun onError(download: Download, error: Error, throwable: Throwable?) {
            synchronized(lock) {
                ...
                if (fetchGroupListenerMap.isNotEmpty()) {
                    val groupId = download.group
                    val fetchGroup =
                        groupInfoProvider.getGroupReplace(groupId, download, Reason.DOWNLOAD_ERROR)
                    fetchGroupListenerMap.values.forEach {
                        val iterator = it.iterator()
                        while (iterator.hasNext()) {
                            val fetchListener = iterator.next().get()
                ...
                fetchListenerMap.values.forEach {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        if (fetchListener == null) {
                            ...
                            fetchListener.onDownloadBlockUpdated(
                                download,
                                downloadBlock,
                                totalBlocks
                            )
        ...
        override fun onPaused(download: Download) {
            synchronized(lock) {
                ...
                if (fetchGroupListenerMap.isNotEmpty()) {
                    val groupId = download.group
                    val fetchGroup =
                        groupInfoProvider.getGroupReplace(groupId, download, Reason.DOWNLOAD_PAUSED)
                    fetchGroupListenerMap.values.forEach {
                        val iterator = it.iterator()
                        while (iterator.hasNext()) {
                            val fetchListener = iterator.next().get()

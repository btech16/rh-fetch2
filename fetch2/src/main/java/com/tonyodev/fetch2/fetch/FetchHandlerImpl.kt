...
class FetchHandlerImpl(
        ...
        return when (downloadInfo.enqueueAction) {
            EnqueueAction.UPDATE_ACCORDINGLY -> {
                if (existingDownload != null) {
                    ...
                    if (downloadInfo.status == Status.COMPLETED && !storageResolver.fileExists(
                            downloadInfo.file
                        )
                    ) {
                        if (createFileOnEnqueue) {
    ...
    override fun insertDownloadInfo(id: Int, url: String, fileName: String, tag: String): Download {
        ...
        copy.id = getUniqueId(download.url, fileName)
        copy.file = fileName
        copy.total = File(copy.file).length()
        copy.tag = tag
        val pair = fetchDatabaseManagerWrapper.insert(copy)
        if (!pair.second) {
            throw FetchException(FAILED_TO_UPDATE_REQUEST)
        }
        fetchDatabaseManagerWrapper.delete(download)
        return pair.first
    }

    override fun updateCompletedDownloadFile(id: Int, newFileName: String, tag: String): Download {
        val download = fetchDatabaseManagerWrapper.get(id)
            ?: throw FetchException(REQUEST_DOES_NOT_EXIST)
        if (download.status != Status.COMPLETED) {
            throw FetchException(FAILED_RENAME_FILE_ASSOCIATED_WITH_INCOMPLETE_DOWNLOAD)
        ...
        download.tag = tag
        download.total = File(download.file).length()
        fetchDatabaseManagerWrapper.update(download)
        return download
    }
